...
The hosted authorization service and repository TypeScript client implement a
tested [OAuth agent-grants profile](/guides/oauth-agent-grants) using PAR,
PKCE `S256`, DPoP sender constraints, RFC 9207 issuer validation, rotating
refresh tokens, token exchange, and revocation. This is self-assessed
implementation evidence, not independent certification or IETF endorsement.

Grantex is owned by Orchestrum Technologies LLP. Inventor and owner:
Sanjeev Kumar. See [Ownership](/ownership) for the canonical ownership and
contact notice.
...
|---|---:|
| TypeScript | `@grantex/sdk@0.5.0` |
| Python | `grantex==0.4.1` |
| Go | `github.com/mishrasanjeev/grantex-go@v0.2.1` |
