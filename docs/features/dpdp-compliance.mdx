...
Your application can retrieve a data principal's active consents through the
authenticated API or SDK and render its own end-user experience:
...
| Feature | Description |
|---------|-------------|
| **Submission** | Your authenticated application submits grievances through the API or SDK |
| **Routing** | Your application is responsible for notifying and routing work to the configured contact |
| **Deadline tracking** | Your application is responsible for regulatory response-deadline monitoring |
| **Status updates** | DPO can update status: `open`, `investigating`, `resolved`, `escalated` |
| **Escalation** | Your compliance process must implement any required escalation workflow |
...
| `baseUrl` | `string` | `https://api.grantex.dev` | Auth service URL |
