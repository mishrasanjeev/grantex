...
```bash cURL
curl -X PATCH https://api.grantex.dev/v1/anomaly/rules/velocity_spike \
...
```bash cURL
curl -X POST https://api.grantex.dev/v1/anomaly/rules \
  -H "Authorization: Bearer $GRANTEX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "ruleId": "email_flood",
...
```bash cURL
# Acknowledge
curl -X POST https://api.grantex.dev/v1/anomaly/alerts/alert_01.../acknowledge \
  -H "Authorization: Bearer $GRANTEX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"note": "Investigating"}'

# Resolve
curl -X POST https://api.grantex.dev/v1/anomaly/alerts/alert_01.../resolve \
  -H "Authorization: Bearer $GRANTEX_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
GET /v1/anomaly/metrics?window=7d
```
