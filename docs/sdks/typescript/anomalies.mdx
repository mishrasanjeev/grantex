...
```typescript
const current = await grantex.anomalies.getResponsePolicy();
await grantex.anomalies.setResponsePolicy('alert_only');
```

The default `revoke_agent_grants` mode revokes active grants for an implicated agent on a high/critical finding. `alert_only` persists findings without this detector's automatic revocation; it does not disable other controls. Check the installed SDK version before using these source-only methods.
...
<ResponseField name="autoRevokedGrants" type="number">
