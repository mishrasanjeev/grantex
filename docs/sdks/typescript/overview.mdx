...
Published release: **`@grantex/sdk@0.5.0`**. Source candidate `0.5.1` adds the
audited refresh-recovery and retry behavior described below; do not install it
until [Release Status](/release-status) records registry verification.
...
<Note>
SDK `0.5.0` includes `PrepaidWalletAgentClient`,
