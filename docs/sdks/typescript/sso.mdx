...
sidebarTitle: "SSO"
description: "Enterprise OIDC and SAML single sign-on, plus the LDAP direct-bind preview, with multi-IdP connections and domain enforcement."
---

## Overview

The `sso` sub-client manages OIDC and SAML 2.0 enterprise connections,
...
<ParamField body="protocol" type="'oidc' | 'saml' | 'ldap'" required>
  The SSO protocol. Use `"oidc"` for OpenID Connect, `"saml"` for SAML 2.0, or
  `"ldap"` for the current LDAP direct-bind preview.
</ParamField>
...
console.log(result.email);        // null
console.log(result.name);         // null
console.log(result.groups);       // []
console.log(result.mappedScopes); // defaultScopes only
console.log(result.developerId);  // 'dev_01HXYZ...'
