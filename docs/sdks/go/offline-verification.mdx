...
| `ProofJKT` | `string` | No | Thumbprint of the key the caller proved possession of; the token's `cnf.jkt` must match. The verifier does not check DPoP proofs. |
| `RequireProofOfPossession` | `bool` | No | Fail closed unless `ProofJKT` is set and matches `cnf.jkt`. |
| `StandardClaimsOnly` | `bool` | No | Ignore legacy claim aliases and require `typ: at+jwt`. `false` in 0.6: an alias is read when the standard claim is absent. |
| `OnLegacyClaim` | `func(alias, standard string)` | No | Called for each legacy alias a verification relied on; by default a deprecation message is logged once per alias. |
| `Algorithms` | `[]string` | No | Narrow the accepted algorithms to a subset of `GrantTokenAlgorithms()` (`RS256`, `ES256`, the default). Any other value returns a `TokenError`. |

Grant tokens are signed with RS256 or ES256. The key is the JWK Set entry named by
`kid`, and it must be an RSA public key for RS256 or an EC P-256 public key for
ES256, published without a different `alg` and with `use` absent or `sig`.
`alg: none` and HS256 are always rejected.
...
| `AuthorizationDetails` | `[]interface{}` | The `authorization_details` claim |
| `Act` | `*ActorClaim` | RFC 8693 actor chain (delegated grants) |
| `Cnf` | `map[string]interface{}` | Confirmation claim, e.g. `jkt` |
| `Audience` | `[]string` | The `aud` claim |
| `LegacyClaimsUsed` | `[]string` | Legacy aliases read because the standard claim was absent |
