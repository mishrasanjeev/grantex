...
tokenResp, err := client.Tokens.Refresh(ctx, grantex.RefreshTokenParams{
    ...
    IdempotencyKey: refreshAttemptID,
})
...
| `IdempotencyKey` | `string` | No | Stable 16-256 character logical-refresh identifier. Generated and cached in-process when omitted; persist it for restart/failover recovery. |
