...
with Grantex(api_key="gx_live_...") as client:
    entry = client.audit.log(
        agent_id="agt_abc123",
        agent_did="did:grantex:agt_abc123",
...
| `agent_did` | `str`                  | Yes      | --          | The agent's decentralized identifier.    |
| `grant_id` | `str`                   | Yes      | --          | The grant under which the action was performed. |
| `principal_id` | `str`               | Yes      | --          | The principal on whose behalf the action ran. |
...
```python
entry = client.audit.get("alog_abc123")
