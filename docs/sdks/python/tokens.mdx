...
Refresh tokens are single-use and rotated on every refresh per SPEC §7.4. Refresh does not extend the grant lifetime; after `expires_at`, the caller must re-authorize. If the HTTP response is lost after commit, retry the same previous refresh token with the same idempotency key. During a five-minute window, Grantex returns the already-rotated refresh token instead of rotating again while the grant remains active. After that window, or once the rotated child token has been used, the previous token is rejected.
...
with Grantex(api_key="gx_live_...") as client:
    # Persist this with the old token until the response is durable.
    refresh_attempt_id = str(uuid4())
    new_token = client.tokens.refresh(RefreshTokenParams(
        refresh_token="rt_01HXYZ...",
        agent_id="agt_abc123",
        idempotency_key=refresh_attempt_id,
    ))
...
| `idempotency_key` | `str` | No | Stable 16-256 character identifier for this logical refresh. The SDK retains an omitted key for five minutes in-process; persist an explicit key for restart/failover recovery. |
