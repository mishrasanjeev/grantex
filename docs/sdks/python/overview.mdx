...
Published release: **`grantex==0.5.1`**. PyPI metadata, distribution contents,
...
<Note>
  Version `0.5.1` is a patch release. `enforce()` applies the tightest budget
  cap and denies malformed, negative, or non-finite caps and amounts (calls that
  earlier versions allowed with such values are now denied); the FastAPI
  enforcer reads the `Authorization` header; grant-token verification caches
  the JWKS and runs off the event loop; resource ids are percent-encoded in
  request paths; single-use authorization codes are never retried; and event
  streams bound connect time.
</Note>

<Note>
  Version `0.5.0` adds EVM payment responses and authenticated principal/agent
