...
Published release: **`grantex==0.4.1`**. PyPI metadata, distribution contents,
...
<Note>
  Version `0.4.1` adds bounded refresh lost-response recovery and audited retry
  behavior to the `WalletSpendPoliciesClient`,
  `PrincipalPrepaidWalletClient`, `AgentPrepaidWalletClient`, and
  `generate_dpop_key` capabilities introduced in `0.4.0` for layered policy,
  exact approvals, reload governance, and ES256 DPoP agent wallet calls. See [Agent Wallet
