...
Repository source adds `get_response_policy()` and
...
published PyPI baselines may not include these methods yet. Alert-only retains
...
| `response_mode` | `str`              | Account response mode for this run.     |
| `auto_revoked_grants` | `int`         | Grants revoked by this detector.        |
