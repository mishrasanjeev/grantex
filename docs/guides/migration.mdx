...
<Steps>
  ...
  <Step title="Enable audit logging">
    ...
    await grantex.audit.log({
      ...
      status: 'success',
      metadata: { resource: `event:${eventId}` },
    });
    ```
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Replace token introspection with local JWKS verification">
    ...
    // Claims are verified locally after the standalone helper fetches JWKS
    ```
  </Step>
...
- [Token Verification](/guides/token-verification) — Online and local JWKS-based verification
