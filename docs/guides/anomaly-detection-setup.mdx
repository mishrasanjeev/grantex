...
curl -H "Authorization: Bearer $GRANTEX_API_KEY" \
  https://api.grantex.dev/v1/anomaly/rules
```
</CodeGroup>
...
```bash
curl -X PATCH https://api.grantex.dev/v1/anomaly/rules/off_hours_activity \
...
```bash cURL
curl -X POST https://api.grantex.dev/v1/anomaly/channels \
  -H "Authorization: Bearer $GRANTEX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "slack",
...
1. **Check that rules are enabled** -- `GET /v1/anomaly/rules` and verify `enabled: true`
...
3. **Check channel status** -- `GET /v1/anomaly/channels` and verify `enabled: true`
