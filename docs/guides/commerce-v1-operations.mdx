...
Live payment mode remains unavailable. Plural remains blocked until exact API and webhook signature
details are confirmed and legal, compliance, and operations reviews are complete.
...
- Failed webhook replay is marked `blocked`; metadata-only listing is available for operator review.
...
6. Keep live Plural disabled and Plural sandbox marked blocked until API and webhook contracts are confirmed.
...
- Plural API and webhook signature blockers are resolved or explicitly accepted as mock-only sandbox scope.
...
- Plural API or webhook signature scheme remains unconfirmed for the target pilot.
...
- Failed webhook replay is required but safe raw payload storage and redaction are not implemented.
- Emergency re-enable is required but the reviewed recovery API contract does not exist.
- Backup/restore/RPO review is required for the pilot but not approved.

## CSV Product And Variant Import Columns

Commerce V1 currently exposes API-backed product and variant creation. CSV import remains an
operator workflow specification until a dedicated importer is implemented.
...
Importer safety requirements before implementation:
...
- Plural remains blocked until exact API behavior and error mapping are confirmed.
...
- A safe failed webhook listing and replay API needs an explicit V1 contract before implementation.
  Ops health reports this as `webhook_failed_event_list_and_replay_api_contract_deferred`.
  Required fields are tenant, merchant, payment intent, provider key, provider event ID, safe status,
  safe error code, timestamps, and operator audit references. Raw provider payload replay requires a
  storage and redaction design that is not part of M6A or M6C. Do not fake replay from partial data.
...
5. Communicate sandbox or pilot status to the operator team.

Re-enable decision:

- The emergency disable API remains one-way in M6C. There is no re-enable route in the V1 API.
- Re-enabling agentic commerce requires a reviewed recovery contract with operator authorization,
  incident review, policy version and activation evidence, and audit semantics. Until that contract
  exists, dashboard re-enable controls must remain unavailable.

## Legal And Live Mode Blockers

Before enabling real Plural or live payments, require human signoff for:

- Confirmed Plural API contract, credential fields, idempotency behavior, and webhook signature scheme.
