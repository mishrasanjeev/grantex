...
- JWKS is exempt from Fastify rate limiting.
...
These route-local Fastify limits replace the 5,000 requests per minute Fastify default on those
routes; they are not layered with that default. Commerce routes use custom authentication and do
not consume the standard developer API-key plan bucket. Caller-keyed budgets per merchant, agent,
or provider remain an open operations refinement for production quota isolation.
