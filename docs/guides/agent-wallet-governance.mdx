...
For the internal `grantex:prepaid` rail, blocking an assignment, a wallet, or all of one agent's wallets releases active
...
Base USDC signatures have a different boundary: new signatures stop, but issued
signatures remain valid on-chain until used or expired. Their reservations stay
held and count against limits until finalized-chain reconciliation. See
[Base USDC custody](/guides/base-usdc-custody) before enabling real funds.
...
| External issuer/custody adapter | Opt-in native Base USDC adapter included in source; other/unconfigured rails fail closed | Provision protected custody and RPC using the Base guide, or integrate the chosen provider; refunds/disputes are not added by this adapter |
