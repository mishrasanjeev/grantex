...
Use mock-provider/internal sandbox flows for development. The Plural sandbox
adapter can be exercised only with `PLURAL_SANDBOX_ENABLED=true`, stored sandbox
credentials, and scrubbed evidence. Production Commerce V1, live payments, and
live Plural are disabled. Approved temporary Option A smoke uses isolated Cloud
Run, Cloud SQL, Redis, smoke secrets, and mock provider resources that are
deleted immediately after evidence capture.
...
Plural live flag, and every readiness acknowledgement. When any acknowledgement
is absent, live provider paths fail closed with `live_readiness_blocked`.
...
This guard is a release blocker, not a launch instruction. Public docs, demos,
and AgenticOrg handoffs must continue to describe mock-provider evidence and
clearly labeled sandbox-provider evidence only until the live packet is approved
and verified.
...
- Public docs that imply AP2, UCP, ACP, Plural live mode, payment-provider, or
  external pilot certification.
