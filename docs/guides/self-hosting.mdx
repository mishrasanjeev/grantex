...
| `JWT_RETIRED_PUBLIC_KEYS` | No | — | JWK Set of public keys that must still verify after rotation |
| `SIGNING_KEY_STORE` | No | `env` | `env` or `postgres` (generated, stored encrypted, rotated with `dist/cli/rotate-signing-key.js`) |
...
The JWK Set publishes every platform signing key with `kid`, `alg` and `use: "sig"`, including keys
kept for verification after rotation.

- **Postgres key store** (`SIGNING_KEY_STORE=postgres`): run `node dist/cli/rotate-signing-key.js [--alg ES256]`.
  The old key stays published for `SIGNING_KEY_RETIRED_GRACE_SECONDS`.
- **Env key store**: publish the new key first (configure it under the other algorithm's setting),
  then switch `JWT_SIGNING_ALG`, then move the old public JWK to `JWT_RETIRED_PUBLIC_KEYS` and remove
  the old private key. See `docs/self-hosting.md` Section 7 for the full procedure.
