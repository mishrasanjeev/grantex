...
|----------|-------|--------|
| **All endpoints** (global IP guard) | 500 req | 1 minute |
...
When a client exceeds its limit, the API returns `429 Too Many Requests`; `Retry-After` gives the minimum seconds to wait. Rate-limited responses expose `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset` (seconds until reset). The allow-listed JWKS endpoint is exempt and need not include those headers.
