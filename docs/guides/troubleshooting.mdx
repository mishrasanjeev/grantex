...
3. Reduce request frequency:
   - **Use local JWKS-based verification** instead of `POST /v1/tokens/verify`
     when a revocation lookup is not required. The standalone helpers still
     fetch JWKS per call, but that endpoint is exempt from rate limits.
...
| Endpoint | Limit |
|----------|-------|
| Default Fastify IP policy (routes without an override) | 5,000 req/min |
| Standard API-key - Free plan | 100 req/min per developer |
| Standard API-key - Pro plan | 500 req/min per developer |
| Standard API-key - Enterprise plan | 2,000 req/min per developer |
...
|---------------|-------|-----|
| "Refresh token already used" | Token was already used and the idempotency key changed or was lost, the five-minute window expired, its child was used, or the grant expired | Retry only with the same old token and persisted idempotency key; otherwise use the newest stored token or re-authorize |
