...
| `@grantex/sdk@0.5.0` and `@grantex/x402@0.3.0` | Applications install managed-wallet clients from npm | These are repository release candidates; npm publication remains separate from server deployment | Keep using published `0.4.1` and `0.2.0` until the exact candidate versions are registry-verified, then complete the server, custody, notification, and merchant dependencies below |
...
Release candidates prepared August 31, 2026: repository source provides SDK
`0.5.0` and x402 `0.3.0`; registry publication is not inferred from the source
tree. Treat the registry as authoritative and pin exact versions only after
verification. See [Release
...
Python `0.4.0` must pass source import, Ruff, strict Mypy, Pytest, distribution
...
$PyPI = Invoke-RestMethod 'https://pypi.org/pypi/grantex/0.4.0/json'
if ($PyPI.info.version -ne '0.4.0') { throw 'PyPI verification failed.' }
```
...
the exact source, and pushing annotated tag `v0.2.0`. A tag in the Grantex
...
```powershell
npm view '@grantex/sdk@0.5.0' version dist.integrity --json
npm view '@grantex/x402@0.3.0' version dist.integrity --json
python -m pip download --no-deps --dest $env:TEMP 'grantex==0.4.0'
go list -m -json 'github.com/mishrasanjeev/grantex-go@v0.2.0'
```
