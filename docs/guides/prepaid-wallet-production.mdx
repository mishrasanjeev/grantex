...
verified funding and reconciliation monitoring. Python `0.5.1` and Go `v0.3.0`
...
$PyPI = Invoke-RestMethod 'https://pypi.org/pypi/grantex/0.5.1/json'
if ($PyPI.info.version -ne '0.5.1') { throw 'PyPI verification failed.' }
```
...
instructions to overwrite those immutable versions.
...
python -m pip download --no-deps --dest $env:TEMP 'grantex==0.5.1'
