...
| `@grantex/sdk@0.5.1` and `@grantex/x402@0.3.0` | Applications install managed-wallet clients from npm | x402 `0.3.0` is published; SDK `0.5.1` is a source release candidate pending clean registry verification | Continue using published SDK `0.5.0` until `0.5.1` is verified, then pin the exact versions and complete the dependencies below |
...
Published and clean-install verified August 31, 2026: SDK `0.5.0` and x402
`0.3.0`. Registry publication is not inferred from the source tree, so treat
