...
| Claim | Description |
|-------|-------------|
| `act` | RFC 8693 actor chain. `act.sub` is the parent agent that created this delegation; a deeper delegation nests the parent token's `act` inside, so the chain reads from the most recent delegator to the first |
