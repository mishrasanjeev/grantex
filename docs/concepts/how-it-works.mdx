...
- **Locally verifiable** — Services verify signatures and claims with published
  JWKS keys, without a per-token call to the Grantex verification API. The JWKS
  endpoint must remain reachable whenever a verifier needs current keys.
- **Compliance-supporting controls** — Audit, scope, consent, and revocation
  features can provide evidence for applicable frameworks. They do not by
  themselves determine legal compliance or a system's risk classification.
