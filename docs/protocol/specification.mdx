...
`alg` is `RS256` or `ES256` (one per deployment; RS256 by default). Verifiers use an explicit
allowlist of those two and select the JWK Set key by `kid`, of the key type the algorithm requires.
...
Standard claims: `iss`, `sub`, `aud` (when bound), `exp`, `iat`, `jti`, `client_id`, `scope`
(space-delimited), `cnf.jkt` (when key-bound), `act` (RFC 8693 delegation chain) and
`authorization_details` (RFC 9396: purpose, tools, caps, budget, decision references). Grant record
fields are under `urn:grantex:grant`. The full profile is `spec/grant-token-0.6.md`.

### 6.3 Legacy Claim Aliases

Issued next to the standard claims while `GRANT_TOKEN_LEGACY_CLAIMS` is on (default on in 0.6, off in 0.7).
Verifiers prefer the standard claim and reject a token whose claim and alias disagree.
...
| `bdg` | budget `authorization_details` entry |
...
4. `scope` (space-delimited) contains the required scopes for the requested operation
...
- Tokens MUST be signed with RS256 or ES256. HS256 and `alg: none` MUST be rejected, and a key MUST NOT be used for an algorithm other than the one it is published for.
