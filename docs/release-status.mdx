...
| Python SDK | `grantex==0.4.0` | Release candidate; current published release is `0.3.14` | Python 3.9+ |
| Go SDK | `github.com/mishrasanjeev/grantex-go@v0.2.0` | Release candidate; current published release is `v0.1.10` | Go 1.26.1+ |
...
<Note>
The four primary wallet SDK candidates must be independently registry-verified
after publication. Installing them does not provision external
...
<Info>
  ...
  wallet clients across TypeScript, Python, and Go. Standard developer API-key
  plan throughput and custom-auth quotas remain separate from SDK publication.
</Info>
...
- **Python application:** use published `grantex==0.3.14` until candidate `0.4.0` is registry-verified.
- **Go application:** use published `github.com/mishrasanjeev/grantex-go@v0.1.10` until candidate `v0.2.0` is proxy-verified.
...
```bash Python (current published)
python -m pip install grantex==0.3.14
```

```bash Go (current published)
