...
| TypeScript SDK | `@grantex/sdk@0.5.0` | Release candidate; current published release is `0.4.1` | Node.js 18+; ESM |
| x402 Payment Protocol | `@grantex/x402@0.3.0` | Release candidate; current published release is `0.2.0`; external custody remains operator-supplied | Node.js 20+; ESM |
| Python SDK | `grantex==0.4.0` | Published to PyPI and fresh-install verified | Python 3.9+ |
| Go SDK | `github.com/mishrasanjeev/grantex-go@v0.2.0` | Tagged, Go-proxy verified, and fresh-install tested | Go 1.26.1+ |
...
<Note>
The TypeScript and x402 wallet SDKs remain prepared npm candidates. Python
`0.4.0` and Go `v0.2.0` were independently registry-verified after publication.
Installing any SDK does not provision external
...
<Info>
  ...
  wallet clients across TypeScript, Python, and Go. Python `0.4.0` and Go
  `v0.2.0` are now published; the TypeScript and x402 candidates remain pending
  on npm. Standard developer API-key plan throughput and custom-auth quotas
...
- **TypeScript application:** use published `@grantex/sdk@0.4.1` until candidate `0.5.0` is registry-verified.
- **x402 prepaid-wallet application:** use published `@grantex/x402@0.2.0` with `@grantex/sdk@0.4.1` until candidates `0.3.0` and `0.5.0` are registry-verified; keep external custody disabled until an operator adapter passes review.
- **Python application:** use published `grantex==0.4.0`; it includes developer, principal, and ES256 DPoP agent wallet clients.
- **Go application:** use proxy-verified `github.com/mishrasanjeev/grantex-go@v0.2.0`; it includes corrected Agent/Audit contracts and layered wallet clients.
...
```bash TypeScript (current published)
npm install @grantex/sdk@0.4.1
```

```bash x402 (current published)
