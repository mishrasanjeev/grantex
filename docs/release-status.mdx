...
| Python SDK | `grantex==0.4.0` | `0.4.1` | Candidate publication pending | Python 3.9+ |
| Go SDK | `github.com/mishrasanjeev/grantex-go@v0.2.0` | `v0.2.1` | Candidate tag and Go-proxy verification pending | Go 1.26.1+ |
...
<Note>
TypeScript `0.5.1`, Python `0.4.1`, and Go `v0.2.1` are source release
candidates pending registry publication and clean-install verification. x402
`0.3.0` remains published. Installing any SDK does not provision external
...
- **Python application:** use published `grantex==0.4.0` now; upgrade to source candidate `0.4.1` only after PyPI publication and clean-install verification.
- **Go application:** use proxy-verified `github.com/mishrasanjeev/grantex-go@v0.2.0` now; upgrade to source candidate `v0.2.1` only after tag and public proxy verification.
