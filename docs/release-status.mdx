...
| TypeScript SDK | `@grantex/sdk@0.5.0` | `0.5.1` | Candidate publication pending | Node.js 18+; ESM |
| x402 Payment Protocol | `@grantex/x402@0.3.0` | - | Published to npm and fresh-install verified; external custody remains operator-supplied | Node.js 20+; ESM |
| Python SDK | `grantex==0.4.1` | - | Published to PyPI and fresh-install verified | Python 3.9+ |
| Go SDK | `github.com/mishrasanjeev/grantex-go@v0.2.1` | - | Tagged, public Go-proxy verified, and fresh-install verified | Go 1.26.1+ |
...
<Note>
TypeScript `0.5.1` remains a source release candidate pending npm publication
and clean-install verification. Python `0.4.1`, Go `v0.2.1`, and x402 `0.3.0`
are published and registry verified. Installing any SDK does not provision external
...
<Info>
  ...
  `0.4.1`, and Go `v0.2.1` include matching bounded refresh-recovery behavior.
...
- **TypeScript application:** use published `@grantex/sdk@0.5.0` now; upgrade to source candidate `0.5.1` only after npm publication and clean-install verification.
- **x402 prepaid-wallet application:** use published `@grantex/x402@0.3.0` with published `@grantex/sdk@0.5.0`; keep external custody disabled until an operator adapter passes review.
- **Python application:** use PyPI-verified `grantex==0.4.1` for bounded refresh recovery and layered prepaid-wallet governance.
- **Go application:** use public-proxy-verified `github.com/mishrasanjeev/grantex-go@v0.2.1` for bounded refresh recovery, corrected Agent/Audit contracts, and layered prepaid-wallet governance.
...
```bash Python
python -m pip install grantex==0.4.1
```
