...
Before checking `scp`, the wrapper verifies the JWT signature and claims with
the configured JWKS endpoint. The current standalone verifier may fetch keys on
each call, so this is not an unverified decode or a guaranteed network-free hot
path. If a verified token only includes `contacts:read`, calling
`delete_contact` throws before your function executes.
