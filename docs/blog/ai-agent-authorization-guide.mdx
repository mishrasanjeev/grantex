...
// This verifies the JWT via JWKS, then checks its scopes against the manifest
...
`enforce()` fails closed if signature or claim verification fails. It uses the
configured JWKS endpoint before evaluating the manifest, and the current
standalone verifier may fetch those keys on each invocation. A decode-only
helper must never be used as an authorization decision.
