...
Self-hosting is still fully supported, but for teams that want to skip the infrastructure work: the managed cloud is live at `api.grantex.dev`. Sign up via the developer portal, get an API key, and start authorizing agents.
...
| IETF draft revision | -02 published; -03 under review |

## What's Next

The active individual IETF draft (`draft-mishra-oauth-agent-grants-02`) has been narrowed to an OAuth authorization profile; budget controls, event streaming, credential vaults, and external policy backends are implementation-specific extension work. A revision `-03` candidate and implementation report are maintained in the repository. We also published a draft NIST NCCoE comment mapping DAAP capabilities to the AI Risk Management Framework; no public submission receipt or NIST endorsement is claimed.
