...
With agents performing high-stakes actions autonomously, you cannot afford that
window. Grantex supports local signature-and-claim validation after JWKS key
retrieval, and online verification (`POST /v1/tokens/verify`) that checks
revocation state in real time. Current standalone SDK verifiers fetch JWKS per
call. After revocation, the next online verification returns `valid: false`.
...
This is not just a technical nicety. The EU AI Act applies in phases and can
require logging, transparency, and human oversight depending on the system's
classification and the organization's role. Grantex's audit trail, scoped
grants, and revocation controls can support those obligations, but do not by
themselves establish compliance.
