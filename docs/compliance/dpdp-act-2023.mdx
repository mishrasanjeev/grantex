...
Every consent record can be withdrawn through the authenticated DPDP API or SDK.
Your application is responsible for presenting an accessible withdrawal control
to the data principal; Grantex does not currently ship a separate DPDP portal or
one-click withdrawal UI. Withdrawal triggers:
...
// Expose this operation through your own authenticated end-user workflow
```
...
The authenticated DPDP API and SDK can create and retrieve grievance records.
Your application must provide the submission UI, notify or route work to the
responsible contact, and implement any required response-deadline monitoring.
Grantex does not currently provide a grievance form, DPO routing, or SLA timers.
...
Grantex exposes authenticated APIs and SDK methods for listing consent records,
withdrawing consent, requesting erasure, generating data exports, and filing
grievances. The data fiduciary must integrate those operations into its own
verified end-user workflow and implement rights not represented by a Grantex
endpoint. There is no separate Grantex DPDP data-principal portal.
