...
<Note>
The prepaid-wallet API and SDK shown here are implemented in
`@grantex/sdk@0.5.0`, `@grantex/x402@0.3.0`, Python `grantex==0.4.0`, and Go
`v0.2.0`. Python and Go are published and registry-verified. The TypeScript and
x402 versions remain prepared npm candidates, so keep using their prior
published versions until the exact candidates are registry-verified.
