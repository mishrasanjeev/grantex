...
<Note>
The prepaid-wallet API and SDK shown here are implemented in the
`@grantex/sdk@0.5.0`, `@grantex/x402@0.3.0`, Python `grantex==0.4.0`, and Go
`v0.2.0` release candidates. Keep using the prior published versions until the
exact candidate is registry-verified. `sandbox_ledger` is a complete
local ledger mode.
