...
The protocol is documented as active individual Internet-Draft `draft-mishra-oauth-agent-grants`. Revision `-02` is published, and a `-03` submission candidate is maintained in `docs/ietf-draft/`. Individual drafts are not IETF-endorsed standards and require working-group adoption before entering the working-group standards process.
