...
| TypeScript SDK | @grantex/sdk 0.3.11 | Primary TypeScript client package. |
| Python SDK | grantex 0.3.12 | Primary Python client package. |
| OpenAPI | 0.1.3 | Needs reconciliation with SDK versions. |
| MCP Auth | @grantex/mcp-auth 2.0.1 | Independently versioned package. |

## Package Versions

The repository contains 29 packages under `packages/`. Each row maps a directory to the published artifact name and the version recorded in its manifest at the time of writing.

| # | Directory | Published name | Version | Status |
| ---: | --- | --- | ---: | --- |
| 1 | `packages/sdk-ts` | @grantex/sdk | 0.3.11 | Primary SDK (TypeScript) |
| 2 | `packages/sdk-py` | grantex | 0.3.11 | Primary SDK (Python) |
...
| 11 | `packages/crewai` | grantex-crewai | 0.1.6 | Adapter |
| 12 | `packages/openai-agents` | grantex-openai-agents | 0.1.5 | Adapter |
| 13 | `packages/google-adk` | grantex-adk | 0.1.5 | Adapter |
| 14 | `packages/strands-py` | grantex-strands | 0.1.1 | Adapter |
| 15 | `packages/strands` | @grantex/strands | 0.1.0 | Adapter |
| 16 | `packages/express` | @grantex/express | 0.1.4 | Middleware |
| 17 | `packages/fastapi` | grantex-fastapi | 0.1.4 | Middleware |
| 18 | `packages/gateway` | @grantex/gateway | 0.1.4 | Gateway |
| 19 | `packages/conformance` | @grantex/conformance | 0.1.7 | Test suite |
| 20 | `packages/adapters` | @grantex/adapters | 0.1.4 | Service adapters |
| 21 | `packages/destinations` | @grantex/destinations | 0.1.2 | Event destinations |
| 22 | `packages/dpdp` | @grantex/dpdp | 0.1.0 | Compliance controls |
| 23 | `packages/gemma` | @grantex/gemma | 0.1.0 | Offline authorization (TS) |
| 24 | `packages/gemma-py` | grantex-gemma | 0.1.0 | Offline authorization (Python) |
| 25 | `packages/a2a` | @grantex/a2a | 0.1.3 | A2A bridge (TS) |
| 26 | `packages/a2a-py` | grantex-a2a | 0.1.3 | A2A bridge (Python) |
| 27 | `packages/mpp` | @grantex/mpp | 0.1.1 | MPP support |
| 28 | `packages/x402` | @grantex/x402 | 0.1.1 | x402 support |
| 29 | `packages/terraform-provider-grantex` | terraform-provider-grantex | Go module (1.21) | Terraform provider |
