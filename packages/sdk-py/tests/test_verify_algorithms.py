...
# spec/examples/signing-keys-legacy-kid.json is generated by the auth service:
# its JWK Set on 2026-09-01 and tokens signed by its keys, including a pre-0.6
# token signed on 2026-08-31 with kid grantex-2026-08.
_SERVED = json.loads(
    (Path(__file__).resolve().parents[3] / "spec" / "examples" / "signing-keys-legacy-kid.json").read_text(
        encoding="utf-8"
    )
)


@pytest.mark.parametrize("name", sorted(_SERVED["tokens"]))
def test_tokens_verify_against_the_jwk_set_the_auth_service_serves(mocker: Any, name: str) -> None:
    _serve(mocker, _SERVED["jwks"]["keys"])
    grant = verify_grant_token(
        _SERVED["tokens"][name]["token"],
        VerifyGrantTokenOptions(jwks_uri=f"{_SERVED['issuer']}/.well-known/jwks.json"),
    )
    assert grant.grant_id == _SERVED["claims"]["grnt"]
    assert grant.principal_id == _SERVED["claims"]["sub"]
