...
This module is a verbatim copy of ``grantex.canonical`` from the decision-grant
canonicalisation work (spec/canonicalization.md), kept private to
``grantex.evidence`` until that module is released. The two must stay
identical: once ``grantex.canonical`` is on main this copy is replaced by a
re-export of it, and ``tests/test_evidence_canonical.py`` checks that they
agree whenever ``grantex.canonical`` is importable.
