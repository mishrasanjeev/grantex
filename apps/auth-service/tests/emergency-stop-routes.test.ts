...
describe('POST /v1/emergency-stop', () => {
  ...
  it('reports the blast radius without revoking anything for a dry run', async () => {
    state.handlers.push([/SELECT id FROM grants\s+WHERE agent_id =/, [{ id: 'grnt_1' }, { id: 'grnt_2' }]]);
    const res = await app.inject({
  ...
  it('records the stop and cascades over every matched grant', async () => {
    state.handlers.push([/SELECT id FROM grants\s+WHERE agent_id =/, [{ id: 'grnt_1' }]]);
    ...
    expect(res.statusCode).toBe(200);
    const stop = res.json<{ stopId: string; grantsRevoked: number; agentsStopped: string[] }>();
    ...
    expect(state.statements.some((s) => s.includes('UPDATE emergency_stops SET grants_revoked'))).toBe(true);
...
describe('POST /v1/admin/emergency-stop', () => {
  ...
  it('stops a whole developer, taking the developer from the scope', async () => {
    ...
    state.handlers.push([/SELECT id FROM grants WHERE developer_id =/, [{ id: 'grnt_1' }]]);
    state.handlers.push([/WITH RECURSIVE tree/, [
    ...
    expect(res.statusCode).toBe(200);
    expect(res.json()).toMatchObject({ developerId: 'dev_TENANT', grantsRevoked: 1, dryRun: false });
  });
});
